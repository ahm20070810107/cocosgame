package com.huson.cocosgame.util.server.handler;
...
import com.huson.cocosgame.core.engine.game.ActionTaskUtils;
import com.huson.cocosgame.core.engine.game.state.GameEvent;
  ...
  //抢地主事件
    ...
    {  
		...
		if(!Strings.isBlank(token)){
			 ...
			 */
			Token userToken ;
			if(beiMiClient!=null && !Strings.isBlank(token) && (userToken = (Token) CacheHelper.getApiUserCacheBean().getCacheObject(token, beiMiClient.getOrgi()))!=null){
				//鉴权完毕
