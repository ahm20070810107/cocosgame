...
import com.huson.cocosgame.web.service.repository.jpa.RoleAuthRepository;
import com.huson.cocosgame.web.service.repository.jpa.RoleRepository;
...
import com.huson.cocosgame.web.service.repository.jpa.UserRoleRepository;
...
public class RoleController extends Handler{
    ...
    public ModelAndView userroledelete(HttpServletRequest request ,@Valid String id , @Valid String role) {
		UserRole userRole  = userRoleRes.getOne(id);
    ...
    public ModelAndView delete(HttpServletRequest request ,@Valid Role role) {
    	...
    	if(role!=null){
			userRoleRes.findByOrgiAndRole(super.getOrgi(request), role).forEach(value->userRoleRes.delete(value));
    ...
    public ModelAndView authsave(HttpServletRequest request ,@Valid String id ,@Valid String menus) {
    	...
    	roleAuthList.forEach(value ->roleAuthRes.delete(value) );
    	if(!Strings.isBlank(menus)){
