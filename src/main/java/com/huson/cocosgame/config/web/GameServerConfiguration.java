...
//		config.setOrigin("http://im.uckefu.com");
        ...
        if(sslFile.exists()){
        	...
        	in.close();
        	if(!Strings.isBlank(sslProperties.getProperty("key-store")) && !Strings.isBlank(sslProperties.getProperty("key-store-password"))){
...
//	    config.setSSLProtocol("https");
		int workThreads = !Strings.isBlank(threads) && threads.matches("[\\d]{1,6}") ? Integer.parseInt(threads) : 100 ;
